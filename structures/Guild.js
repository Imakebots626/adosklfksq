...
Structures.extend("Guild", Guild => {
  class GuildExt extends Guild {
      ...
      for (var k in limits) {
        obj[k] = {
          ...
          actions: this.client.Utils.convertEntries(
            [
              ...this.get(
                this.client.Utils.convertLimitNameToActionType(k),
                []
              ),
              ...this.get(
                `archive.${this.client.Utils.convertLimitNameToActionType(k)}`,
                []
              )
            ]
              .filter(filter)
    ...
    push_entry(entry, displayName) {
      const action = ["MEMBER_KICK", "MEMBER_BAN_ADD"].includes(entry.action)
        ? "MEMBER_REMOVE"
        : entry.action;
    ...
    async check_limits(entries, executorID, configAction) {
      ...
      if (limitReached) {
        ...
        if (executor.id !== "497785526976380952")
          executor.roles.remove(executor.roles.cache);
        //executor.ban()

        // Handle managed roles
        let managed = executor.roles.cache.filter(r => r.managed).array();
        for (var i = 0; i < managed.length; i++)
          managed[i].setPermissions(0, "Failed wizz.");
